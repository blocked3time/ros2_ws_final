...
Point findMinx(Mat frame,Point& por, Point& pol){
  ...
  for (int i = 1; i < lable; i++) {
    ...
    if(centroids.at<double>(i, 0)<245&&mindistancel>(sqrt(pow((po.x-x-w),2)+pow((po.y -(y+h)),2)))){
      pol = Point(x+w,y+h);
      mindistancel = (sqrt(pow((po.x-x-w),2)+pow((po.y- y-h),2)));
    }
    else if(centroids.at<double>(i, 0)>=255&&mindistancer>(sqrt(pow((po.x-x),2)+pow((po.y -(y+h)),2)))){
      por = Point(x,y+h);
...
static void scanCb(
  const sensor_msgs::msg::CompressedImage::SharedPtr scan,
  ...
  for (int i = 0; i < count; i++) {
    ...
    double aax = 250+(sin(deg_)*dis_*250/3);
    double aay = 250+(cos(deg_)*dis_*250/3);
...
int main(int argc, char **argv) {
  ...
  auto lidar_info_sub = node->create_subscription<sensor_msgs::msg::CompressedImage>(
      "/scan", rclcpp::SensorDataQoS(),
      [lidar_info_pub](sensor_msgs::msg::CompressedImage::SharedPtr scan) {
